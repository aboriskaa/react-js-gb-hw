...
export default function SignInSide() {
	...
	useEffect(() => {
		const check = function () {
			if (email.length > 3 && password.length > 3 && validateEmail(email)) {
	...
	useEffect(() => {
		const check = function () {
			if (errorInState === 'auth/user-not-found') {
				setError('User not found, please Sign Up!');
			} else if (errorInState === 'auth/wrong-password') {
	...
	if (userInState) return <div>closed</div>;
	else
		return (
						...
						>
							...
							>
								...
								<Grid container>
									...
									<Grid item>
										<Link
											onClick={(event) => handlerRouter('/register', event)}
											href='#'
											variant='body2'
										>
